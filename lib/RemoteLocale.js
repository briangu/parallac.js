...
function createFnRequest(sessionId, fn, completeFn) {
  ...
  // writeln("createFnRequest", sessionId, Object.keys(outstandingRequests).length)
  return payload
}

function completeRequest(data) {
  // writeln("completeRequest", data)
...
var RemoteLocale = function (config, id) {
  ...
  socket.on('connect_failed', function (data) {
    console.log("connect_failed:", config.URI)
  })
  ...
  function done() {
    // writeln("done", "here", id, Object.keys(outstandingRequests).length)
  ...
  function on(sessionId) {
    // console.log("RemoteLocale", "on", sessionId)
    ...
    return {
      with: function (obj) {
        ...
        // HACK: TOTAL HACK! to get things off the ground
        // TODO: we need real RPC here
        // TODO: return promise
        socket.emit('pushContext', {
          sessionId: sessionId,
          context: obj
        })
        return this;
