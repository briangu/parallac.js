...
var RemoteLocale = function (config, id) {
  ...
  config.writeLambdaFn = config.remote.writeLambdaFn || defaultWriteLambdaFn
  config.writeLogFn = config.remote.writeLogFn || defaultWriteLogFn
  ...
  socket.on('writeFn', config.writeLambdaFn);
  socket.on('writeln', config.writeLogFn);
