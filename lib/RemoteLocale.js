...
function createSessionRequest(sessionId, onCompleteFn) {
  ...
  // debug("createSessionRequest", sessionId, Object.keys(outstandingRequests).length)
  return payload
}

function createFnRequest(sessionId, subContext, fn, onCompleteFn) {
  const requestId = uuid.v4()
  const payload = {
    sessionId: sessionId,
    requestId: requestId,
  ...
  return {
    ...
    createSessionContext: function (sessionId) {
      ...
      return connected
        .then(() => {
          return new Promise(function (resolve, reject) {
            socket.emit('createSessionContext', createSessionRequest(sessionId, (result) => {
