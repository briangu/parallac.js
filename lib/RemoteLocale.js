...
const defaultWriteLambdaFn = function (data) {
  let fn = eval(data.fn)
  fn.apply(null, JSON.parse(data.args))
}

const defaultWriteLogFn = function (data) {
...
var RemoteLocale = function (config, id) {
  ...
  // console.log("RemoteLocale", "config", config)

  let writeLambdaFn = config.remote.writeLambdaFn || defaultWriteLambdaFn
  let writeLogFn = config.remote.writeLogFn || defaultWriteLogFn
  ...
  socket.on('writeFn', writeLambdaFn);
  socket.on('writeln', writeLogFn);
