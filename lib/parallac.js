...
var Locale = function (localeConfig, id, parentContext) {
  ...
  function createLocaleSession(id) {
    let sessionLocale = new Locale(localeConfig, id, context())
    localeSessions[id] = sessionLocale
    return sessionLocale
  }

  // function createLocaleSessionWithContext(sessionContext) {
  //   id = uuid.v4()
  //   const childContext = createChildContext(context(), context)
  //   let sessionLocale = new Locale(localeConfig, id, childContext)
  //   sessions[id] = sessionLocale
  //   return sessionLocale
  // }

  function closeLocaleSession(id) {
    delete localeSessions[id]
  }
  ...
  function on() {
    ...
    const onId = sessionCount++ //uuid.v4()
    return {
      with: function (obj) {
        // sessions[onId] = sessions[onId] || {}
        // Object.assign(sessions[onId], obj)
        sessions[onId] = obj
        return this;
      },
      do: function (fn) {
        return new Promise(function (resolve, reject) {
          try {
            ...
            let sessionContext = sessions[onId]
            ...
            console.log("do", "try", "error", err, fn.toString())
            reject(err);
          }
        })
        .then((results) => {
          delete sessions[onId]
          ...
          if (kindOf === '[object Array]') {
            ...
            if (innerKindOf === '[object Promise]') {
              console.log("wrapping results with Promise.all")
  ...
  return {
    ...
    createSessionContext: function (id) {
      return Promise.resolve(createLocaleSession(id))
    },
    closeSessionContext: function (id) {
      return Promise.resolve(closeLocaleSession(id))
    }
  }
}

function on(locale) {
  // console.log("on", "locale", locale.id, locale.on)
  return locale.on()
}
...
var DistArray = function (domain) {
  ...
  return {
    ...
    init: function() {
      var calls = domain.locales.map((locale) => locale.setSymbol(objId, {}))
    ...
    set: function (i, v) {
      var dom = this.domain.get(i)
      // console.log("set", "dom", dom)
      return on(this.domain.get(i))
        .with({
          i: i,
          v: v,
