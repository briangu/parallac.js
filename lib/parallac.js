...
function DomainIterator(ranges) {
  let indexes = ranges.map((r) => r[0])
  ...
  return {
    next: function () {
      if (this._done) {
        ...
        copyArray(this._indexes, this._value)
        this._done = incrementIndexes(ranges, this._indexes)
        return {
          value: this._value
        }
      }
...
function defaultRangeMapper(locales, ranges, coords) {
  return locales[hashCode(coords) % locales.length]
}

var Domain = function (locales, ranges, fnRangeMapper) {
  ...
  return {
    ...
    get: function (coords) {
      return fnRangeMapper(locales, ranges, coords)
    },
    [Symbol.iterator]: function () {
      return DomainIterator(ranges)
    }
  }
}
...
function createDomain(locales, len) {
  var d = new Domain(locales, [[0,len-1]])
  return d.init()
}
