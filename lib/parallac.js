...
function DomainIterator(dom, length) {
  return {
    next: function () {
      if (this._idx < length) {
        return {
          value: this._idx++ // return the domain entry
        }
      } else {
        return {
          done: true
...
var Domain = function (locales, len) {
  ...
  return {
    ...
    init: function () {
      ...
      return new Promise(function (resolve, reject) {
        for (let i = 0; i < len; i++) {
          domainIndices.push(locales[i % locales.length])
        }
        debug("domain:domain", domainIndices)
        resolve(k)
      })
    },
    get: function (x) {
      return domainIndices[x]
    },
    [Symbol.iterator]: function () {
      return DomainIterator(this, len)
    }
  }
}
