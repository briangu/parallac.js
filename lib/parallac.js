...
function createRemoteLocale(localeConfig, i, baseContext) {
  var remoteLocale = new RemoteLocale(localeConfig, i, baseContext)
...
function createSession(config, sessionId) {
  ...
  return Promise.all(calls)
    .then((sessionLocales) => {
      sessionConfig.here = sessionLocales[sessionConfig.here.id]
...
function closeSession(config) {
  console.log("closing session", config.sessionId)
  var calls = config.Locales.map((locale) => locale.closeSessionContext(config.sessionId))
  return Promise.all(calls)
    .catch((err) => {
...
function run(fn, localeConfig) {
  return init(localeConfig)
    .then(createSession)
    .then((sessionConfig) => {
      // console.log("run", "here", sessionConfig.here.context().i)
      return on(sessionConfig.here)
        .do(fn)
        .catch((err) => {
          // console.log("run", "error", err)
          closeSession(sessionConfig)
          throw err
        })
        .then((result) => {
