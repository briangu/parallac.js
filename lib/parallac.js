...
function loadConfig(localeConfig) {
  ...
  if (!localeConfig.locales) {
    ...
    localeConfig.locales = URIs.map((URI) => {
      let newLocaleConfig = Object.assign({}, localeConfig)
      newLocaleConfig.URI = URI
      return newLocaleConfig
    })
  }
...
function defaultWriteFn(fn) {
  console.log("defaultWriteFn", "fn", fn.toString())
  Array.prototype.shift.apply(arguments)
...
function createLocalLocale(localeConfig, i, baseContext) {
  ...
  locale.context().writeFn = defaultWriteFn
  locale.context().writeln = console.log
  locale.context().__dirname = __dirname
  locale.context()._sys = {}
