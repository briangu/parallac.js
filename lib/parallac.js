...
function createRemoteLocale(i, baseContext) {
  var remoteLocale = new RemoteLocale(i, baseContext)
    ...
    .then((config) => {
      ...
      for (let i = 0; i < config.locales.length; i++) {
        ...
        if (localeConfig.URI === hereConfig.URI) {
          let localLocale = createLocalLocale(i, BaseContext)
...
function createSession(config, sessionId) {
  ...
  var calls = config.Locales.map((locale) => locale.createSessionContext(sessionId))
  return Promise.all(calls)
    .then((sessionLocales) => {
      if (config.here) {
        config.here.context().Locales = sessionLocales
      }
      return config
    })
}

function closeSession(config, sessionId) {
  console.log("closing session", sessionId)
  var calls = config.Locales.map((locale) => locale.closeSessionContext(sessionId))
  return Promise.all(calls)
}

// TODO: move session management outside of the locales
    ...
    .then((config) => {
      return createSession(config)
        .then((sessionLocales) => {
          ...
          config.here = sessionLocales[config.here.id]
          config.Locales = sessionLocales
          return config
        })
    })
    .then((config) => {
      on(config.here).do(fn)
        .catch((err) => {
